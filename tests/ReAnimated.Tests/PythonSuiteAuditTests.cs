...
public sealed class PythonSuiteAuditTests
{
    ...
    public void ReviewedManifestClassifiesEveryExactPythonRegression()
    {
        ...
        string? pythonOracleRoot = Environment.GetEnvironmentVariable(
            "DLR_PYTHON_ORACLE_ROOT");
        if (!string.IsNullOrWhiteSpace(pythonOracleRoot))
        {
            string resolvedOracleRoot = Path.GetFullPath(
                Environment.ExpandEnvironmentVariables(
                    pythonOracleRoot));
            Assert.True(
                Directory.Exists(resolvedOracleRoot),
                $"Configured Python oracle root does not exist: {resolvedOracleRoot}");
            AssertPythonSourcesMatch(
                resolvedOracleRoot,
                manifestRoot);
        }
    }

    private static void AssertPythonSourcesMatch(
        string repository,
        JsonElement manifest)
    {
        JsonElement[] sources = RequiredProperty(
                manifest,
                "pythonTestSources")
            .EnumerateArray()
            .ToArray();
        string testsDirectory = Path.Combine(repository, "tests");
        string[] currentPaths = Directory.EnumerateFiles(
                testsDirectory,
                "test_*.py",
            ...
            .ToArray();
        string[] expectedPaths = sources
            .Select(source => RequiredString(source, "path"))
            .ToArray();
        Assert.Equal(
        ...
        string repositoryPrefix =
            Path.GetFullPath(repository).TrimEnd(
