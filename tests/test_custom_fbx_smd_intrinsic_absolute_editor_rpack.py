...
def assets() -> dict[str, Path]:
    root = _asset_root()
    values = {
        "animation_fbx": root / "external-animation-control.fbx",
        "source_rest_fbx": root / "external-animation-control.fbx",
...
def test_custom_frame0_preserves_source_pose_instead_of_target_bind(
    ...
    for row in rows:
        # This experiment intentionally begins in full-body control frame 0,
