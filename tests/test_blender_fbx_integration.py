...
def test_bundled_native_fbx_uses_readable_helpers_and_roundtrips(tmp_path: Path) -> None:
    ...
    # Zero-length twist nodes use a display-only grandparent so Blender does
    # not shorten the visible upper arm or thigh by half.
    assert document.parent_by_name["r_uparmtwist"] == "r_clavicle"
    assert document.parent_by_name["r_thightwist"] == "pelvis"
