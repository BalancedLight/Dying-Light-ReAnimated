...
The dedicated **ANM2 → FBX** workspace converts extracted animations back into editable skeleton FBXs. Native export supports any matching `.crig`, including doors and props; cross-rig mode provides conservative automatic mapping plus manual review. An opt-in 87-node DL1 player rig exposes 18 named helper bones for metadata-validated FBX editing and exact reimport while leaving the legacy 69-bone target as the default. FBX-declared source cadence, ANM2 sampling cadence, and playback cadence are tracked separately; reverse export can resample between independent ANM2/FBX rates and consumes hash-validated `.anm2.dlrmeta.json` timing provenance.

For helper editing and lossless reimport, follow the
[ANM2-to-FBX-to-ANM2 round-trip and Blender best-practices
guide](docs/ANM2_TO_FBX.md#quick-anm2-to-fbx-to-anm2-round-trip).
