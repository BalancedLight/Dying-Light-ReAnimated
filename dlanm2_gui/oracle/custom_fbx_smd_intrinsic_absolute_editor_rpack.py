...
The uploaded `external-animation-control.fbx` now matches the Blender-derived trusted rest matrices across all 65 bones. The custom candidates then transfer corrected animated limb directions **absolutely in body space** rather than forcing animation frame 0 to equal the Dying Light bind pose.
...
Custom candidates intentionally do **not** start at target bind. Frame 0 should look like frame 0 of `external-animation-control.fbx`. The target-bind resource remains a separate control.
...
Validation against the matching Blender-derived `external-animation-control.fbx` oracle:
