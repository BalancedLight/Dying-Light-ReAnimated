# Documentation index

## Everyday use

- [GUI quick start](GUI_GUIDE.md)
- [Humanoid retargeting](RETARGETING.md)
- [Chrome Rig custom targets](CHROME_RIGS.md)
- [Animation-script targets](ANIMATION_SCRIPT_TARGETS.md)
- [RPack workflow](RPACK_WORKFLOW.md)
- [Root motion and IK](ROOT_MOTION_AND_IK.md)
- [Troubleshooting](TROUBLESHOOTING.md)

## Advanced reference

- [ANM2 format](ANM2_FORMAT.md)
- [Project format](PROJECT_FORMAT.md)
- [Windows EXE build](BUILDING_WINDOWS_EXE.md)
- [Developer architecture](DEVELOPER_ARCHITECTURE.md)

Contributor guidance is under `docs/project/`. Generated diagnostics and scratch research are not bundled with the release tree.
